...
其中 `RandomCrop` 可以增强模型对物体位置变化的鲁棒性，`RandomHorizontalFlip` 可以扩充图像姿态变化，降低过拟合风险。验证集和测试集不使用随机增强，只进行尺寸统一和标准化，以保证评估结果稳定。
...
从目前训练日志看，BasicCNN 的训练 loss 和验证 loss 均呈下降趋势，验证准确率逐步提升到约 `60%`，说明该网络已经有效学习到 STL-10 的类别特征，并且没有出现明显过拟合。因此它可以作为课程项目中的基础模型结果。
...
```text
第 1 轮:
train_loss = 1.9413, train_accuracy = 0.2247
valid_loss = 1.6360, valid_accuracy = 0.3133

第 30 轮:
train_loss = 0.9529, train_accuracy = 0.6533
valid_loss = 0.9287, valid_accuracy = 0.6676

验证集最高准确率:
valid_accuracy = 0.6676, epoch = 30
```

训练过程中，训练集 loss 从约 `1.94` 下降到约 `0.95`，验证集 loss 从约 `1.64` 下降到约 `0.93`。同时，训练集准确率从约 `22.47%` 提升到约 `65.33%`，验证集准确率从约 `31.33%` 提升到约 `66.76%`。这说明模型在训练过程中持续学习到了有效的图像分类特征。

从曲线形态看，训练集和验证集的 loss 都整体下降，二者之间没有出现明显分离；训练集和验证集准确率也同步上升，最终验证集准确率略高于训练集准确率。这种现象是合理的，因为训练阶段使用了 `RandomCrop`、`RandomHorizontalFlip` 和 `Dropout`，训练样本难度更高；验证阶段不使用随机增强，也不会启用 Dropout，因此验证准确率可能略高。

因此，当前 BasicCNN **没有明显过拟合**。模型更接近于稳定收敛状态：后期准确率提升变慢，但验证集性能仍然维持在较高位置，没有出现验证 loss 持续上升或验证 accuracy 明显下降的情况。
...
```text
[[84  4  1  1  1  0  1  0  3  5]
 [ 4 53  0 14  7  7  0 15  0  0]
 [ 2  1 80  1  0  2  5  0  1  8]
 [ 0  9  0 44  6  6  2 31  1  1]
 [ 0  5  2  6 73  4  4  6  0  0]
 [ 0  3  0  9 17 27 12 32  0  0]
 [ 0  2  0  0 13 14 68  3  0  0]
 [ 0  2  0  9 10 11  0 67  0  1]
 [ 9  5  2  0  0  0  0  0 79  5]
 [ 3  0 12  1  2  1  2  0  4 75]]
```

从结果看，测试集准确率为 `65.00%`，与验证集最高准确率 `66.76%` 比较接近，说明验证集结果能够较好地反映模型在未见数据上的表现，模型泛化能力较稳定。
...
```text
cat 被预测为 monkey: 31 次
dog 被预测为 monkey: 32 次
dog 被预测为 deer:   17 次
horse 被预测为 dog:  14 次
horse 被预测为 deer: 13 次
```

这说明 BasicCNN 已经能较好地区分外观差异较大的类别，但对于姿态、纹理和背景相似的动物类别，特征表达能力仍然有限。
